...
export function getOffchainMessageEnvelopeEncoder(): VariableSizeEncoder<OffchainMessageEnvelope> {
    return transformEncoder(
        ...
        envelope => {
            ...
            const signerAddresses = getSignerAddresses(envelope.content);
            ...
            const orderedSignatureMap: OffchainMessageSignaturesMap = {};
            for (const address of signerAddresses) {
...
function decodePartiallyDecodedOffchainMessageEnvelope(
    ...
    const signerAddresses = getSignerAddresses(content);
...
function getSignerAddresses(content: ReadonlyUint8Array): Address[] {
    const signerAddressesDecoder = getHiddenPrefixDecoder(
        ...
        [
            ...
            createDecoder({
                read(bytes, offset) {
                    const [_, newOffset] = getOffchainMessageVersionDecoder().read(bytes, offset);
                    return [undefined, newOffset];
                },
            }),
        ],
    );

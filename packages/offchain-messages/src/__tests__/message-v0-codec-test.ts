...
describe('getOffchainMessageV0Decoder()', () => {
    ...
    it('decodes a well-formed ASCII encoded message according to spec', () => {
        ...
        expect(decoder.decode(encodedMessage)).toStrictEqual({
            ...
            requiredSigners: [{ address: SIGNER_A }, { address: SIGNER_B }],
            version: 0,
        });
    });
    it('freezes the decoded message', () => {
        ...
        expect(decodedMessage.requiredSigners).toBeFrozenObject();
        decodedMessage.requiredSigners.forEach(signer => expect(signer).toBeFrozenObject());
    });

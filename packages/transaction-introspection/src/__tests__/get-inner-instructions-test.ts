...
import { walkInnerInstructionsFromMeta } from '../get-inner-instructions';
...
describe('walkInnerInstructionsFromMeta', () => {
    ...
    it('yields nothing when meta has no inner instructions', () => {
        expect([...walkInnerInstructionsFromMeta({}, accountMetas)]).toStrictEqual([]);
        expect([...walkInnerInstructionsFromMeta({ innerInstructions: null }, accountMetas)]).toStrictEqual([]);
    });
    ...
    it('decodes base58 data and resolves indices into AccountMetas', () => {
        ...
        expect(out[0].instruction.programAddress).toBe('program');
        expect(out[0].instruction.accounts).toStrictEqual([
            { address: 'fee-payer', role: AccountRole.WRITABLE_SIGNER },
            { address: 'data-account', role: AccountRole.WRITABLE },
        ]);
        expect(Array.from(out[0].instruction.data)).toStrictEqual([9, 8, 7]);
    });

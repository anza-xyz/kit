import { getLifetimeTokenDecoder, getOptionalLifetimeTokenEncoder } from '../lifetime-token';
...
describe('getOptionalLifetimeTokenEncoder', () => {
    it('should encode a 23 byte base58 string', () => {
        ...
        expect(getOptionalLifetimeTokenEncoder().encode(token)).toStrictEqual(new Uint8Array(32).fill(33));
    });

    it('should encode undefined as 32 zero bytes', () => {
        expect(getOptionalLifetimeTokenEncoder().encode(undefined)).toStrictEqual(new Uint8Array(32));
    });
});

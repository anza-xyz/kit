        ...
        (_key: SWRKey, { next }: SWRSubscriptionOptions<SolanaRpcResponse<TItem>, TError>) => {
            ...
            const unsubscribe = store.subscribe(() => {
                ...
                if (state.status === 'loaded') {
                    // Guard the store/cache boundary: a fresh store's high-water mark starts at -1,
                    // so without this check a remount could regress the cache to an older slot than
                    // it already holds. The bridge fires asynchronously (network/notification), well
                    // after the post-commit sync below has populated `cachedDataRef`, so a warm
                    // remount sees the cached slot here.
                    const cachedSlot = cachedDataRef.current?.context.slot ?? -1n;
                    if (state.data.context.slot < cachedSlot) return;
                    // The kit primitive always emits `SolanaRpcResponse<TItem>` — pass it through.
                    next(null, state.data);
                } else if (state.status === 'error') {
                    next(state.error as TError);
                }
            });
            store.connect();
            return () => {

...
// to component unmount or SWR supersession — SWR's model is to discard the stale result rather
// than cancel the network call).
...
export function useRequestSWR<T, TError = Error>(
    ...
    const fetcher = useCallback(async (): Promise<T> => {
        ...
        if (typeof current === 'function') {
            return await current(userSignal ?? NEVER_ABORTED_SIGNAL);
        }
        if (userSignal) {

 ...
 *
 * Returns SWR's native `{ data, error }` shape. `data` is a {@link SlotTaggedValue} — flat
 * `{ slot, value }` lifted from the `SolanaRpcResponse` envelope the kit primitive emits. The
 * `value` is the unified item produced by the spec's mappers; `slot` is the envelope's
 * `context.slot`.
 *
...
export function useTrackedDataSwr<TRpcValue, TSubscriptionValue, TItem>(
...
): SWRSubscriptionResponse<SlotTaggedValue<TItem>, unknown> {
    ...
    const subscribe = useCallback((_key: SWRKey, { next }: SWRSubscriptionOptions<SlotTaggedValue<TItem>, unknown>) => {
        ...
        const unsubscribe = store.subscribe(() => {
            ...
            if (state.status === 'loaded') {
                // The kit primitive always emits `SolanaRpcResponse<TItem>` — narrow off
                // the envelope directly. No runtime duck-type detection needed.
                const envelope = state.data as SolanaRpcResponse<TItem>;
                next(null, { slot: envelope.context.slot, value: envelope.value });
    ...
    return useSWRSubscription<SlotTaggedValue<TItem>, unknown, SWRKey>(effectiveKey, subscribe, swrConfig);
}

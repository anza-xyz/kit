...
export type TrackedDataResult<T> = {
    /**
     * The latest value, slot-deduped across the initial RPC and the subscription. `undefined`
     * on the first load or when disabled. On `loading` after a prior outcome, on `error`, and
     * on a subsequent refresh, holds the last known value (if one ever arrived) so UIs can
     * show stale data rather than flashing to blank.
     */
    data: T | undefined;
    /**
    ...
    /**
     * The slot `data` was observed at — lifted from the underlying `SolanaRpcResponse`
     * envelope's `context.slot` and used by the store for dedup. Follows `data`: once a value
     * has arrived from either source, `slot` stays pinned to it across subsequent `loading`
     * and `error` transitions (i.e. it's stale during refresh or after a `loaded → error`,
     * just like `data`). `undefined` before the first value arrives (initial `loading`,
     * `disabled`, or on the server). Read it directly to show "data as of slot X" UIs.
     */
    slot: Slot | undefined;
    /**

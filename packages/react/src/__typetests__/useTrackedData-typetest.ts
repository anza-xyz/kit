...
import type { CreateReactiveStoreWithInitialValueAndSlotTrackingConfig } from '@solana/kit';
...
{
    // Infers `TItem` from the spec's mappers
    useTrackedData(spec) satisfies TrackedDataResult<number>;
    ...
    const { refresh, status } = useTrackedData(spec);
    refresh();

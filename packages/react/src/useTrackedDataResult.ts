 ...
 * {@link TrackedDataResult} shape consumed by `useTrackedData`.
 *
 * Lighter than `useSubscriptionResult` because the envelope shape is fixed at the type level:
 * `data` and `slot` come straight off `state.data.value` / `state.data.context.slot` — no
 * runtime duck-type detection needed.
 *

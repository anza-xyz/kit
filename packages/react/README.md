...
Bridges any async function into a tracked action with `send` / `status` / `data` / `error` / `reset`. Each `send(...)` runs `fn` with a fresh `AbortSignal` and tracks the lifecycle through React state; calling `send` again while a prior call is in flight aborts the first.

`fn` is held in a ref that always points at the latest closure — there is no `deps` array to maintain. Each `send(...)` invokes the most recently rendered `fn`, so values captured inside (e.g. form state, route params) are always fresh. This matches the convention used by `useMutation` in TanStack Query and `useWriteContract` in wagmi. In-flight calls are unaffected — they continue with the closure they captured at dispatch time.
...
`send` returns `Promise<TResult>`. Fire-and-forget callers can ignore it and render from `status` / `data` / `error`. Awaiters that read the resolved value (e.g. to navigate on success) should filter superseded calls with `isAbortError` from `@solana/promises`:

```tsx
try {
    const { id } = await send(body);

...
describe('createTransactionPlanExecutor', () => {
    ...
    describe('parallel scenarios', () => {
        ...
        it('limits the maximum number of concurrent operations when visiting parallel transaction plans', async () => {
            expect.assertions(7);
            const messageA = createMessage('A');
            const messageB = createMessage('B');
            const messageC = createMessage('C');
            ...
            const executeTransactionMessage = jest.fn().mockImplementation(({ id }: { id: string }) => {
                return new Promise(resolve => {
                    resolvers[id] = () => resolve({ transaction: createTransaction(id) });
                });
            });
            ...
            executor(parallelTransactionPlan([messageA, messageB, messageC, messageD])).catch(() => {});

            // The first two operations are executed immediately.
            await jest.runAllTimersAsync();
            expect(executeTransactionMessage).toHaveBeenCalledTimes(2);
            expect(executeTransactionMessage).toHaveBeenCalledWith(messageA, { abortSignal: undefined });
            expect(executeTransactionMessage).toHaveBeenCalledWith(messageB, { abortSignal: undefined });

            // When operation B resolves, the next one is executed.
            resolvers['B']();
            await jest.runAllTimersAsync();
            expect(executeTransactionMessage).toHaveBeenCalledTimes(3);
            expect(executeTransactionMessage).toHaveBeenNthCalledWith(3, messageC, { abortSignal: undefined });

            // When operation A resolves, the last one can be executed.
            resolvers['A']();
            await jest.runAllTimersAsync();
            expect(executeTransactionMessage).toHaveBeenCalledTimes(4);
            expect(executeTransactionMessage).toHaveBeenNthCalledWith(4, messageD, { abortSignal: undefined });

            // All operations can now resolve.
            resolvers['C']();
            resolvers['D']();
        });

        it('flattens the result of parallel plans visited with `maxConcurrentOperations`', async () => {
            expect.assertions(1);
            const messageA = createMessage('A');
            const messageB = createMessage('B');
            const messageC = createMessage('C');
        ...
        it('cancels subsequent operations after at least one plan fails in the current concurrent operations', async () => {
            expect.assertions(2);
            const messageA = createMessage('A');
            const messageB = createMessage('B');
            const messageC = createMessage('C');
            ...
            const executeTransactionMessage = jest.fn().mockImplementation((message: { id: string }) => {
                // eslint-disable-next-line jest/no-conditional-in-test
                return message.id === 'A' ? Promise.reject(cause) : forwardId(message);
            });
            const executor = createTransactionPlanExecutor({
                executeTransactionMessage,
                maxConcurrentOperations: 2,
            ...
            await expectFailedToExecute(
                ...
                new SolanaError(SOLANA_ERROR__INSTRUCTION_PLANS__FAILED_TO_EXECUTE_TRANSACTION_PLAN, {
                    cause,
                    transactionPlanResult: parallelTransactionPlanResult([
                        // First concurrent set resolved.
                        failedSingleTransactionPlanResult(messageA, cause),
                        successfulSingleTransactionPlanResult(messageB, createTransaction('B')),
                        // Subsequent plans canceled.
                        canceledSingleTransactionPlanResult(messageC),
                    ]),
                }),
            );
        ...
        it('does not call `executeTransactionMessage` on subsequently canceled operations', async () => {
            expect.assertions(4);
            const messageA = createMessage('A');
            const messageB = createMessage('B');
            const messageC = createMessage('C');
            ...
            await executor(parallelTransactionPlan([messageA, messageB, messageC])).catch(() => {});
            // First set of operations executed.
            expect(executeTransactionMessage).toHaveBeenCalledTimes(2);
            expect(executeTransactionMessage).toHaveBeenCalledWith(messageA, { abortSignal: undefined });
            expect(executeTransactionMessage).toHaveBeenCalledWith(messageB, { abortSignal: undefined });
            // Subsequent operations canceled.
            expect(executeTransactionMessage).not.toHaveBeenCalledWith(messageC, { abortSignal: undefined });
        });
        ...
        it('can abort parallel transaction plans mid-operations', async () => {
            expect.assertions(2);
            const messageA = createMessage('A');
            const messageB = createMessage('B');
            const messageC = createMessage('C');
            ...
            const executeTransactionMessage = jest.fn().mockImplementation((message: { id: string }) => {
                ...
                if (message.id === 'A') {
                    return new Promise(resolve => {
                        resolveA = () => resolve({ transaction: createTransaction('A') });
                    });
                }
                return FOREVER_PROMISE;
            });
            const executor = createTransactionPlanExecutor({
                executeTransactionMessage,
                maxConcurrentOperations: 2,

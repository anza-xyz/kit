import {
    ...
    SOLANA_ERROR__INSTRUCTION_PLANS__MESSAGE_FACTORY_CANNOT_ACCOMMODATE_PLAN,
    SOLANA_ERROR__INSTRUCTION_PLANS__MISSING_INSTRUCTIONS_IN_INSTRUCTION_PLAN,
 ...
 *
 * @see {@link createBaseTransactionPlanner}
 */
 ...
 * @example
 * ```ts
 * const transactionPlanner = createBaseTransactionPlanner({
 ...
 */
export function createBaseTransactionPlanner(config: TransactionPlannerConfig): TransactionPlanner {
    return async (instructionPlan, { abortSignal } = {}): Promise<TransactionPlan> => {
        ...
        if (!plan) {
            throw new SolanaError(SOLANA_ERROR__INSTRUCTION_PLANS__MISSING_INSTRUCTIONS_IN_INSTRUCTION_PLAN, {
                instructionPlan,
            });
        }
...
async function traverseMessagePacker(
    ...
    while (!messagePacker.done()) {
        ...
        if (!candidate) {
            const message = await createNewMessage(context, instructionPlan, messagePacker.packMessageToCapacity);
...
async function createNewMessage(
    ...
    if (getTransactionMessageSize(updatedMessage) > TRANSACTION_SIZE_LIMIT) {
        throw new SolanaError(SOLANA_ERROR__INSTRUCTION_PLANS__MESSAGE_FACTORY_CANNOT_ACCOMMODATE_PLAN, {
            instructionPlan,
            transactionMessage: updatedMessage,
        });
    }
...
function fitEntirePlanInsideMessage(
    ...
    switch (kind) {
        ...
        case 'single':
            ...
            if (getTransactionMessageSize(newMessage) > TRANSACTION_SIZE_LIMIT) {

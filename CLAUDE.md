...
- **Exact type assertions**: `satisfies` is one-way, so it cannot assert that a type is _exactly_ some type — `5 satisfies number` passes. When a test needs "this is exactly `number`" or "this is exactly this shape", copy the `Equal` helper — **along with its explanatory docblock** — from `packages/plugin-core/src/__typetests__/client-typetest.ts` into the typetest file and assert `true satisfies Equal<Actual, Expected>`. There is no shared home for typetest helpers, so duplicating it per file is intentional.

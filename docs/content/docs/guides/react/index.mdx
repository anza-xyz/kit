...
Some plugins ship React bindings from a `/react` subpath of their own package. Unlike the core hooks, they take the client as an argument instead of reading it from `ClientProvider`, so each hook stays colocated with the plugin that owns its state.

| You want to…                   | Package                           | Guide                                     |
| ------------------------------ | --------------------------------- | ----------------------------------------- |
| Drive a Wallet Standard wallet | `@solana/kit-plugin-wallet/react` | [Wallet hooks](/docs/guides/react/wallet) |

When the client you pass to `ClientProvider` installs a browser wallet plugin, hand that same client to the plugin's hooks. Build it with [`walletSigner()`](/docs/guides/setting-up-signers#use-a-browser-wallet) instead of a generated signer.

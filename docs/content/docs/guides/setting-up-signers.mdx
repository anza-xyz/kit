...
The [`@solana/kit-plugin-wallet`](https://www.npmjs.com/package/@solana/kit-plugin-wallet) package discovers [Wallet Standard](https://github.com/wallet-standard/wallet-standard) wallets in the browser, manages the connection lifecycle, and can install the connected wallet's signer as the `payer`, the `identity`, or both. All of the state and actions live on a `client.wallet` namespace that works from any UI framework — React bindings are covered separately on the [Wallet hooks](/docs/guides/react/wallet) page.

```package-install
@solana/kit @solana/kit-plugin-wallet
```

`walletSigner(...)` is the usual choice for a dApp: it installs `client.wallet` and syncs the connected wallet's signer to both roles. Pass the chain that matches the cluster your RPC plugin talks to — one client targets one chain.
...
Read the wallets the plugin discovered from `client.wallet.getState()` and connect one. Once a signing-capable wallet is connected, `client.payer` and `client.identity` are that wallet's signer, so calls like `client.sendTransaction(...)` prompt the wallet as usual.
...
const client = createClient()
    ...
    .use(solanaDevnetRpc());
// ---cut-before---
const [wallet] = client.wallet.getState().wallets;
...
Reading `client.payer` or `client.identity` **throws** while disconnected, and their values change when the user connects, switches account, or disconnects. Gate anything that reads them on `getState().connected?.signer` so you never read them too early.

The other three variants cover the cases where fee payment and user identity should not both come from the wallet:
...
| `walletPayer`         | wallet signer  | —                 | The wallet pays fees; the identity role is installed separately.          |
| `walletIdentity`      | —              | wallet signer     | A relayer or backend key pays fees; the wallet is the user identity.      |
| `walletWithoutSigner` | —              | —                 | Wallet state only; read `getState().connected?.signer` where you need it. |
...
The plugin persists the last connected account in `localStorage` and silently reconnects on the next page load. The `storage`, `storageKey`, `autoConnect`, and `filter` options adjust that behavior:

```ts twoslash
import { walletSigner } from '@solana/kit-plugin-wallet';

const plugin = walletSigner({
    chain: 'solana:devnet',
    storage: null, // disable persistence
    autoConnect: false, // persist, but never reconnect silently
    filter: w => w.features.includes('solana:signAndSendTransaction'),
});
```

`client.wallet` also exposes `disconnect()`, `selectAccount()`, `signMessage()`, and `signIn()` (Sign In With Solana as connect). Subscribe with `client.wallet.subscribe` to bind the same state to Vue, Svelte, or any other UI library.
...
<Callout type="info">
    ...
    `status` stays `'pending'`, the actions throw, and no storage or registry listeners are touched.
    In the browser the plugin initializes normally.
</Callout>

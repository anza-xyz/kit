...
export type Client = {
  estimateAndSetComputeUnitLimit: <T extends BaseTransactionMessage & TransactionMessageWithFeePayer>(
...
export async function createClient(): Promise<Client> {
  if (!client) {
    ...
    const estimateAndSetComputeUnitLimit = async <T extends BaseTransactionMessage & TransactionMessageWithFeePayer>( // [!code ++]

...
This page will be updated to add references to `rpc()` and `localhostRpc()` plugins as the standard way to add RPC to a client, and to note that `client.rpc` from a plugin-based client is the same `Rpc` object described here.

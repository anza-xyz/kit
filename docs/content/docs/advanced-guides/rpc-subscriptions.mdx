...
This page will be updated to add references to the `rpc()` plugin which also sets up `rpcSubscriptions` on the client, and to note that `client.rpcSubscriptions` from a plugin-based client is the same `RpcSubscriptions` object described here.

...
By default the context object supports three optional base properties that have semantic meaning:
...
You can also add any custom properties you need. Because the context type is entirely yours to declare, intersect `TransactionPlanResultContextWithSignature` into it to keep the three base properties above alongside your own:
...
{/* TODO: Restore `twoslash` on this block once `TransactionPlanResultContextWithSignature` ships. */}
...
>({
    executeTransactionMessage: async (context, message) => {
        // Inside the callback, `context.startedAt` is `number | undefined`.
        context.startedAt = Date.now();
        /* ... sign the message, send the transaction, and return it ... */
    },
});
```
...
type MyContext = TransactionPlanResultContextWithSignature & { startedAt: number };

function logResult(result: SingleTransactionPlanResult<MyContext>) {
    if (isSuccessfulSingleTransactionPlanResult(result)) {
        console.log(result.context.signature); // Guaranteed by TransactionPlanResultContextWithSignature.

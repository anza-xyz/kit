...
The callback returns nothing. Everything it wants to report — the signature, the transaction, anything custom — it stores on the `context` object.
...
const transactionPlanExecutor = createTransactionPlanExecutor({
    executeTransactionMessage: async (context, message) => {
        ...
        context.signature = getSignatureFromTransaction(transaction);
...
- **`signature`**: The transaction signature. Nothing populates this for you — store it yourself, typically via `getSignatureFromTransaction`. An executor that produces transactions its fee payer has not signed can simply leave it out and declare a context type that does not require it.
...
const transactionPlanExecutor = createTransactionPlanExecutor({
    executeTransactionMessage: async (context, message) => {
        ...
        // Store the transaction about to be sent, and its signature.
        context.transaction = transaction;
        context.signature = getSignatureFromTransaction(transaction);
...
A fresh context is created for every transaction message, so it is empty when your callback receives
it and every property on it — base or custom — is optional. That means your callback cannot annotate
its `context` parameter with required properties. If you want a custom property to be guaranteed on
the result, declare it as an explicit type argument to `createTransactionPlanExecutor` instead:
...
>({
    executeTransactionMessage: async (context) => {
        context.startedAt satisfies number | undefined; // Optional inside the callback.
        context.startedAt = Date.now();
        // ...
        context.signature = {} as Signature;
    },
});
```
...
A custom context replaces the default rather than extending it, so intersect
`TransactionPlanResultContextWithSignature` when you want to keep the guarantee that every successful
result carries a `signature`. Nothing is added to your context on your behalf: the context type you
declare is the context type you get, both inside the callback and on the result.

That is what lets an executor sign transactions without sending them. A transaction that its fee
payer has not signed has no signature to report, so declare a context that does not ask for one and
...
const transactionPlanExecutor = createTransactionPlanExecutor<{ transaction: Transaction }>({
    executeTransactionMessage: async (context, message) => {
        // Signed by the authority only. A relayer will pay for and submit it later.
        context.transaction = await signTransactionMessageWithSigners(message);
    },
});
```

Every successful result from this executor carries a `context.transaction` and no `context.signature`
at all.
